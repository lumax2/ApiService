...
import com.stream.nz.model.OverseaClaims;
...
//        int aesKeyLen = Bytes.toInt(aesKeyLenBytes);
    ...
    public static byte[] hexStringToBytes(String hexString){
        if (StrUtil.isEmpty(hexString) || "".equals(hexString)||hexString ==null){
            return null;
        }
    ...
    public static String decryptByPublicAes(String encryptMsg,String maxusJwt)  {
        RsaVO rsaVO = JwtTools.parsePayload(maxusJwt, OverseaClaims.class).getRsaVO();
        RSAResource rsaResource = RsaAesUtils.transferRsaVoToRsaResource(rsaVO);
        try {
            ...
            log.error("server decrypt msg exception", e.getMessage());
            return null;
        }
    }
